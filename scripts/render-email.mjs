...
function itemRow(item, soup = false) {
  ...
  return `<tr><td style="font-size:14px;line-height:1.5;padding:10px 10px 10px 0;border-bottom:1px solid #eee7de;">${label}${escape(displayName(item))}</td><td align="right" valign="middle" style="width:94px;white-space:nowrap;padding:10px 0;border-bottom:1px solid #eee7de;">${original}<span style="display:inline-block;padding:6px 9px;border-radius:14px;background:${soup ? '#ece6dc' : '#f1ece3'};color:#252525;font-size:13px;font-weight:700;">${price}</span></td></tr>`;
}
function card(r) {
  const badge = r.id === 'bluebell' || r.source?.scope?.startsWith('weekly') ? 'TÝŽDENNÉ MENU' : 'DENNÉ MENU';
  const extras = r.closed ? '' : `${r.desserts.length ? `<div style="font-size:12px;font-weight:700;margin-top:16px;">DEZERTY</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0">${r.desserts.map(i => itemRow(i)).join('')}</table>` : ''}${r.notes.map(n => `<div style="font-size:12px;line-height:1.5;color:#666;margin-top:12px;">${escape(n)}</div>`).join('')}`;
